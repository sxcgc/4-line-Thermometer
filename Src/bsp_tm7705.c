...
/* 通道1和通道2的增益,输入缓冲，极性 */
#define __CH1_GAIN_BIPOLAR_BUF	(GAIN_16 | UNIPOLAR | BUF_NO)
#define __CH2_GAIN_BIPOLAR_BUF	(GAIN_16 | UNIPOLAR | BUF_NO)
...
static void TM7705_Send8Bit(uint8_t _data)
{
	...
	for(i = 0; i < 8; i++)
	{
      ...
      // HAL_SPI_Transmit(&hspi1,&_data,1,0x0A);
...
static uint8_t TM7705_Recive8Bit(void)
{
	...
	return read;
	/*uint8_t read = 0;
        HAL_SPI_Receive(&hspi1,&read,1,0x0A);
	return read;*/
}
