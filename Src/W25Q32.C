...
uint32_t Flash_Addr;
uint32_t LastSectorNum;
uint32_t SequenceWriteStart = 1;
uint8_t temp[4];
/**
  * 函数功能: 擦除扇区
...
uint32_t AddrMove(int32_t step)
{
  Flash_Addr +=step;
  return Flash_Addr;
}

void SPI_FALSH_Sequence_Write(uint8_t* data,uint16_t size)
{
  
  uint32_t Addr=0;
  Addr = AddrLocate();
  
  if(SequenceWriteStart  == 1)
  {
    SPI_FLASH_SectorErase(((Addr+size)/Sector_Size)*Sector_Size);
    SequenceWriteStart = 0;
  }
  if((Addr+size)/Sector_Size > LastSectorNum)
  {
    SPI_FLASH_SectorErase( ((Addr+size)/Sector_Size)*Sector_Size);
  }
  LastSectorNum = (Addr+size)/Sector_Size;
...
uint8_t SPI_FLASH_ReadByte(void)
{
  ...
  if(HAL_SPI_TransmitReceive(&hspi2,&d_send,&d_read,1,0x0A)!=HAL_OK)
    d_read=Dummy_Byte;
